...
* `prm06/rawdata/ngs/*/Info`

* `prm05/rawdata/`

### Calculon

* `/groups/umcg-gd/prm03/projects`
...
```{r}
gc_bias <- read.table("../data/testdata.confidential/QXTR_810-Exoom_v3/run01/qc/statistics/data.gc_bias_metrics", comment.char = "#", header = TRUE, sep = "\t")
...
```{r}
multiTable = function(filepath, comment.char) {
  con = file(filepath, "r")
  tableindex=0
  tableCount = 0
  while (TRUE) {
    fileLine = readLines(con, n = 1)
    if (length(fileLine) == 0) {
      break
    }
    if (!(startsWith(fileLine, comment.char))) {
      if (tableindex == 0) {
        print(strsplit(fileLine, "\\\t"))
...
## MultiQC
The multiqc data provides aggregated results of the different bioinformatics tools

```{r}
VCF <- read.table("../data/testdata.confidential/demo_vcf/20196298_2166656_DNA125983_573149_QXTR868_3183841.final.vcf.table", header = T, sep = "\t")


```

```{r}
insertSizesPrm03 <- read.csv("../data/prm03InsertSizesDates.csv", header = TRUE)
insertSizesPrm03$prepkit <- unlist(lapply(insertSizesPrm03$PROJECT_ID, function (x) {
  return(strsplit(toString(x), '-')[[1]][2])
}))

insertSizesPrm05 <- read.csv("../data/prm05InsertSizeDated.csv", header = TRUE)
insertSizesPrm05$prepkit <- unlist(lapply(insertSizesPrm05$PROJECT_ID, function (x) {
  return(strsplit(toString(x), '-')[[1]][2])
}))

insertSizesPrm06 <- read.csv("../data/prm06InsertSizeDated.csv", header = TRUE)
insertSizesPrm06$prepkit <- unlist(lapply(insertSizesPrm06$PROJECT_ID, function (x) {
  return(strsplit(toString(x), '-')[[1]][2])
}))

insertSizes <- rbind(insertSizesPrm03, insertSizesPrm05)

insertSizes <- insertSizes[!duplicated(insertSizes$SAMPLE_ID), ]
```
```{r}
library(ggplot2)
insertSizes$DATE <- as.Date(as.character(insertSizes$DATE), format = "%y%m%d")

insertSizesDatedSub <- subset(insertSizes, DATE > as.Date("2019-01-01"))
ggplot(insertSizesPrm03DatedSub, aes(x=DATE, y=MEDIAN_INSERT_SIZE, colour=prepkit)) +
  geom_point() +
  xlab("Sequencing Date") +
  ylab("Median Insert size") +
  ggtitle("Median insert size over time")

ggplot(subset(insertSizesPrm03DatedSub, prepkit == 'Exoom_v3'), aes(x=DATE, y=MEDIAN_INSERT_SIZE, colour=prepkit)) +
  geom_point() +
  geom_smooth() +
  xlab("Sequencing Date") +
  ylab("Median Insert size") +
  ggtitle("Median insert size over time for Exoom_v5")

ggplot(subset(insertSizesPrm03DatedSub, prepkit == 'ONCO_v4'), aes(x=DATE, y=MEDIAN_INSERT_SIZE, colour=prepkit)) +
  geom_point() +
  geom_smooth() +
  xlab("Sequencing Date") +
  ylab("Median Insert size") +
  ggtitle("Median insert size over time for ONCO_v4")

ggplot(subset(insertSizesPrm03DatedSub, prepkit == 'ONCO_v5'), aes(x=DATE, y=MEDIAN_INSERT_SIZE, colour=prepkit)) +
  geom_point() +
  geom_smooth() +
  xlab("Sequencing Date") +
  ylab("Median Insert size") +
  ggtitle("Median insert size over time for ONCO_v5")

ggplot(subset(insertSizesPrm03DatedSub, prepkit == 'PCS_v5'), aes(x=DATE, y=MEDIAN_INSERT_SIZE, colour=prepkit)) +
  geom_point() +
  geom_smooth() +
  xlab("Sequencing Date") +
  ylab("Median Insert size") +
  ggtitle("Median insert size over time for PCS_v5")

ggplot(subset(insertSizesPrm03DatedSub, prepkit == 'SVP_v2'), aes(x=DATE, y=MEDIAN_INSERT_SIZE, colour=prepkit)) +
  geom_point() +
  geom_smooth() +
  xlab("Sequencing Date") +
  ylab("Median Insert size") +
  ggtitle("Median insert size over time for SVP_v2")
```
```{r}
hsMetrics <- read.csv("../data/gatheredMetrics/hsMetrics.csv", sep = ",", header = TRUE)
hsMetricsWithDates <- subset(hsMetrics, !is.na(DATE))
hsMetricsWithDates$DATE <- as.Date(as.character(hsMetricsWithDates$DATE), format = "%y%m%d")

ggplot(hsMetricsWithDates, aes(x=DATE, y=PF_READS)) +
  geom_point() +
  geom_smooth() +
  xlab("Sequencing date") +
  ylab("Reads passed filter") +
  ggtitle("Reads passed filter over time")
