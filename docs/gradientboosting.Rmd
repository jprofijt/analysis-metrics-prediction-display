...
for (DNANumber in uniqueSamples[-1]) {
  ...
  InBoth = intersect(colnames(start), colnames(dnadata))
  start <- rbind(start[,InBoth], dnadata[,InBoth])
}
...
head(data2)
data2$FractionDuplicate <- data2$PFuniqueReads/data2$PFreads.y
metricsToAnalyze <- subset(data2, select = c(DNA_numbers, Q30, ChimerasPercentage, TargetBasesPct10X, HsLibrarySize, OnBaitVSselected, ClusterDensity, FractionDuplicate, Intensity, MedianSize))
...
```
```{r}
preprepData <- subset(preprepData, select = -c(WORKLIST_PROCESS_STEP_ID, SCANNED)) ## Remove unusable data
preprepData.num <- subset(preprepData, select = -c(SampleName, SURESELECT_QXT_KIT_ID, SURESELECT_QXT_HYB_MOD_BOX1_ID, ROBOTCONTROLE_USER, AMPUREBEADS_ID, ETOH_ID, SURESELECT_TARGET_ENRICHMENT_KIT))
replaceComma <- function(x) {
  return(gsub(",", ".", x))
}
preprepData.num <- as.data.frame(apply(preprepData.num, 2, replaceComma))
preprepData.num <- as.data.frame(apply(preprepData.num, 2, as.numeric))
preprepData.tot <- subset(preprepData, select = c(SampleName, SURESELECT_QXT_KIT_ID, SURESELECT_QXT_HYB_MOD_BOX1_ID, ROBOTCONTROLE_USER, AMPUREBEADS_ID, ETOH_ID, SURESELECT_TARGET_ENRICHMENT_KIT))
preprepData.num.na <- na.omit(preprepData.num)
preprepData.tot2 <- cbind(preprepData.tot, preprepData.num)
preprepData.tot2$SURESELECT_QXT_KIT_ID <- as.factor(preprepData.tot2$SURESELECT_QXT_KIT_ID)
preprepData.tot2$SURESELECT_QXT_HYB_MOD_BOX1_ID <- as.factor(preprepData.tot2$SURESELECT_QXT_HYB_MOD_BOX1_ID)
preprepData.tot2$ROBOTCONTROLE_USER <- as.factor(preprepData.tot2$ROBOTCONTROLE_USER)
preprepData.tot2$AMPUREBEADS_ID <- as.factor(preprepData.tot2$AMPUREBEADS_ID)
preprepData.tot2$ETOH_ID <- as.factor(preprepData.tot2$ETOH_ID)
preprepData.tot2$SURESELECT_TARGET_ENRICHMENT_KIT <- as.factor(preprepData.tot2$SURESELECT_TARGET_ENRICHMENT_KIT)
```

```{r}
ggplot(preprepData.num.na, aes(x=Concentration, y=ChimerasPercentage)) + geom_point()
ggplot(preprepData.num.na, aes(x=A260_280, y=ChimerasPercentage)) + geom_point()
ggplot(preprepData.num.na, aes(x=A260_230, y=ChimerasPercentage)) + geom_point()
ggplot(preprepData.num.na, aes(x=CONCENTRATION, y=ChimerasPercentage)) + geom_point()
ggplot(preprepData.num.na, aes(x=MOLARITY, y=ChimerasPercentage)) + geom_point()
ggplot(preprepData.tot2, aes(x=SURESELECT_QXT_KIT_ID, y=ChimerasPercentage)) + geom_point()
ggplot(preprepData.tot2, aes(x=SURESELECT_QXT_HYB_MOD_BOX1_ID, y=ChimerasPercentage)) + geom_point()
ggplot(preprepData.tot2, aes(x=ROBOTCONTROLE_USER, y=ChimerasPercentage)) + geom_boxplot()
ggplot(preprepData.tot2, aes(x=AMPUREBEADS_ID, y=ChimerasPercentage)) + geom_point()
ggplot(preprepData.tot2, aes(x=ETOH_ID, y=ChimerasPercentage)) + geom_point()
ggplot(preprepData.tot2, aes(x=SURESELECT_TARGET_ENRICHMENT_KIT, y=ChimerasPercentage)) + geom_point()
```
```{r}
ggplot(preprepData.num.na, aes(x=Concentration, y=Q30)) + geom_point()
ggplot(preprepData.num.na, aes(x=A260_280, y=Q30)) + geom_point()
ggplot(preprepData.num.na, aes(x=A260_230, y=Q30)) + geom_point()
ggplot(preprepData.num.na, aes(x=CONCENTRATION, y=Q30)) + geom_point()
ggplot(preprepData.num.na, aes(x=MOLARITY, y=Q30)) + geom_point()
ggplot(preprepData.tot2, aes(x=SURESELECT_QXT_KIT_ID, y=Q30)) + geom_point()
ggplot(preprepData.tot2, aes(x=SURESELECT_QXT_HYB_MOD_BOX1_ID, y=Q30)) + geom_point()
ggplot(preprepData.tot2, aes(x=ROBOTCONTROLE_USER, y=Q30)) + geom_boxplot()
ggplot(preprepData.tot2, aes(x=AMPUREBEADS_ID, y=Q30)) + geom_point()
ggplot(preprepData.tot2, aes(x=ETOH_ID, y=Q30)) + geom_point()
ggplot(preprepData.tot2, aes(x=SURESELECT_TARGET_ENRICHMENT_KIT, y=Q30)) + geom_point()
```
```{r}
