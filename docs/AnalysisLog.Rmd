...
training.data <- model.data2$Used %>% 
  createDataPartition(p = 0.8, list = FALSE)

train.data  <- model.data2[training.data, ]
test.data <- model.data2[-training.data, ]
...
```{r}
model2pred <- predict (model3,test.data)
confusionMatrix (model2pred,  test.data$Used)
imp <- varImp(model3)$importance

imp$metric <- rownames(imp)
imp$metric <- factor(imp$metric, levels = imp$metric[order(imp$Overall)])

imp <- varImp(model2)$importance

imp$metric <- rownames(imp)
imp$metric <- factor(imp$metric, levels = imp$metric[order(imp$Overall)])
imp$Invluence_level <- ifelse(imp$Overall > 10, "10% or more", "Less than 10%")
...
p <- ggplot(imp, aes(x=metric, y=Overall, fill=Invluence_level)) + geom_bar(stat = "identity")  + coord_flip() + ylab("Level of importance (%)") + xlab("Metric in model") + ggtitle("Metric importance acording to the xdgboost model", subtitle = subt) + scale_fill_manual(values=c("green", "red"))

ggsave("Model_performance1.png", 
       plot = p, 
       dpi = 300)
...
```{r}
# reduce skewedness
model.data2.not.reinserted <- model.data2[model.data2$Used == "Not Reinserted",]
model.data2.reinserted <- model.data2[model.data2$Used == "Reinserted",]
...
```{r}
model4pred <- predict (model4,test.data2)
confusionMatrix (model4pred,  test.data2$Used)
imp2 <- varImp(model4)$importance

imp2$metric <- rownames(imp2)
imp2$metric <- factor(imp2$metric, levels = imp2$metric[order(imp2$Overall)])
imp2$Invluence_level <- ifelse(imp2$Overall > 10, "10% or more", "less than 10%")

library(ROCR)
 predicted.classes <- model4 %>% predict(test.data2)
...
p <- ggplot(imp2, aes(x=metric, y=Overall, fill=Invluence_level)) + geom_bar(stat = "identity") + theme(legend.position = "none") + coord_flip() + ylab("Level of importance (%)") + xlab("Metric in model") + ggtitle("Metric importance acording to the xdgboost model", subtitle = subt) + scale_fill_manual(values=c("green", "red"))
...
metric.list <- complete.data.edit %>% select(UniqueID,A260_280, A260_230, Intensity, PrePhasing, Phasing, ClusterDensity, Average_Q30, ClusterPF, OnBaitVSselected, MeanTargetCoverage, Fold80BasePenalty, HetSNPsensitivity, ChimerasPercentage, ExcMapQPct, PFmismatchRate, FractionDuplicates, MeanSize, PFindelRate, HsPenalty20X, MeanBaitCoverage, SingletonsPercentage, MedianSize, TargetBasesPct20X, Instrument)
...
ggsave(
  "NextSeq_threshold260:230_change.png",
  plot = mean.lines,
  limitsize = TRUE,
  scale = 1,
  width = 14,
...
me.prephasing <- median(metric.list.NextSeq$PrePhasing)
...
metric.list.NextSeq.scaled$threshold <- factor(metric.list.NextSeq.scaled$threshold, levels = c("High", "Norm", "Low"))
df_melted = melt(metric.list.NextSeq.scaled[-c(2:3)][c(1,3,4,2,5:23)], id.vars = c('UniqueID', 'threshold'), na.rm = TRUE)
df_melted
mean.lines <- ggplot(df_melted, aes(x = variable, y = value, fill = threshold)) +
  geom_boxplot(alpha = 0.5) +
...
me.phasing <- median(metric.list.NextSeq$Phasing)
...
metric.list.NextSeq.scaled$threshold <- factor(metric.list.NextSeq.scaled$threshold, levels = c("High", "Norm", "Low"))
df_melted = melt(metric.list.NextSeq.scaled[c(1,6,5,7:25)], id.vars = c('UniqueID', 'threshold'), na.rm = TRUE)
df_melted
mean.lines <- ggplot(df_melted, aes(x = variable, y = value, fill = threshold)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_manual(values=c("red", "green", "blue"), name = "Threshold", labels = c("Above", "Normal", "Below")) +
  ggtitle("Mean scaled metric change", subtitle = "compared to the Phasing higher threshold") + 
  ylab("Change from center") + 
  theme(axis.text.x=element_text(angle = 90, size = 15),
        plot.title = element_text(size = 20),
        plot.subtitle = element_text(size = 15),
...
metric.list.NextSeq.scaled$threshold <- factor(metric.list.NextSeq.scaled$threshold, levels = c("High", "Norm", "Low"))
df_melted = melt(metric.list.NextSeq.scaled[c(1,7,5,6,4, 8:21)], id.vars = c('UniqueID', 'threshold'), na.rm = TRUE)
df_melted
mean.lines <- ggplot(df_melted, aes(x = variable, y = value, fill = threshold)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_manual(values=c("red", "green", "blue"), name = "Threshold", labels = c("Above", "Between", "Below")) +
...
medi <- median(metric.list.NextSeq$Average_Q30)
metric.list.NextSeq$Aver
...
metric.list.NextSeq$th
metric.list.NextSeq.outlier <- metric.list.NextSeq
metric.list.NextSeq.outlier[metric.list.NextSeq$threshold == "High",][-c(1,21)] <- as.data.frame(apply(metric.list.NextSeq[metric.list.NextSeq$threshold == "High",][-c(1,21)], 2, remove_outliers_norm))

metric.list.NextSeq.outlier
metric.list.NextSeq.outlier[metric.list.NextSeq$threshold == "Norm",][-c(1,21)] <- as.data.frame(apply(metric.list.NextSeq[metric.list.NextSeq$threshold == "Norm",][-c(1,21)], 2, remove_outliers_norm))


metric.list.NextSeq.outlier[metric.list.NextSeq$threshold == "Low",][-c(1,21)] <- as.data.frame(apply(metric.list.NextSeq[metric.list.NextSeq$threshold == "Low",][-c(1,21)], 2, remove_outliers_norm))

metric.list.NextSeq.scaled <- na.omit(metric.list.NextSeq.outlier)[-2]
metric.list.NextSeq.scaled[-c(1,20)] <- as.data.frame(apply(metric.list.NextSeq.scaled[-c(1,20)], 2, function(x) {
  rescale(x, to=c(-1,1))
}))


metric.list.NextSeq.scaled$threshold <- factor(metric.list.NextSeq.scaled$threshold, levels = c("Norm", "Low"))
df_melted = melt(metric.list.NextSeq.scaled[-c(2)][c(1,6,3,4,2,5,7:19)], id.vars = c('UniqueID', 'threshold'), na.rm = TRUE)
df_melted
mean.lines <- ggplot(df_melted, aes(x = variable, y = value, fill = threshold)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_manual(values=c("green", "blue"), name = "Threshold", labels = c("Normal", "Below")) +
  ggtitle("Mean scaled metric change", subtitle = "compared to the Q30 threshold lower than median: 90.025 %") + 
  ylab("Change from center") + 
  theme(axis.text.x=element_text(angle = 90, size = 15),
        plot.title = element_text(size = 20),
        plot.subtitle = element_text(size = 15),
...
complete.data.edit$FractionDuplicates <- (1 - complete.data.edit$PFuniqueReads/complete.data.edit$TotalReads.x)
metric.list <- complete.data.edit %>% select(UniqueID,A260_280, A260_230, ClusterDensity, Phasing, PrePhasing, Intensity, TargetBasesPct20X, OnBaitVSselected, PFmismatchRate, ExcMapQPct, MedianTargetCoverage, HetSNPsensitivity, StrandBalance, StandardDeviation, ChimerasPercentage, PFUQaligned, MeanSize, FractionDuplicates, Instrument)
...
sd.ClusterDensity <- sd(metric.list.NextSeq$ClusterDensity)
me.ClusterDensity <- mean(metric.list.NextSeq$ClusterDensity)
metric.list.NextSeq$threshold <- unlist(lapply(metric.list.NextSeq$ClusterDensity, function(x, th=sd.ClusterDensity, center=me.ClusterDensity) {
  val <- x
  if (val > (center + (th* 2))) {
    return("High")
  }
...
metric.list.NextSeq.scaled$threshold <- unlist(lapply(metric.list.NextSeq$ClusterDensity, function(x, th=sd.ClusterDensity, center=me.ClusterDensity) {
  val <- x
  if (val > (center + (th* 2))) {
    return("High")
  }
...
mean.lines <- ggplot(df_melted, aes(x = variable, y = value)) + geom_line(aes(color = threshold, group = UniqueID)) + scale_color_manual(values = c("#f22116", "#16f2b7", "#49f216")) + 
  theme(axis.text.x=element_text(angle = 90)) + ggtitle("Mean scaled metric change", subtitle = "compared to the metrics exceeding 2 * SD (NextSeq)") 

mean.lines

ggsave(
  "NextSeq_threshold_change_ClusterDensity.png",
  plot = mean.lines,
  dpi = 300,
  limitsize = TRUE,
)
...
complete.data.edit$FractionDuplicates <- (1 - complete.data.edit$PFuniqueReads/complete.data.edit$TotalReads.x)
metric.list <- complete.data.edit %>% select(UniqueID,A260_280, A260_230, Phasing, PrePhasing, Intensity, ClusterDensity, TargetBasesPct20X, OnBaitVSselected, PFmismatchRate, ExcMapQPct, MedianTargetCoverage, HetSNPsensitivity, StrandBalance, StandardDeviation, ChimerasPercentage, PFUQaligned, MeanSize, FractionDuplicates, Instrument)
...
metric.list.NextSeq.scaled <- as.data.frame(apply(metric.list.NextSeq, 2, range02))
sd.TargetBasesPct20X <- sd(metric.list.NextSeq$TargetBasesPct20X)
me.TargetBasesPct20X <- mean(metric.list.NextSeq$TargetBasesPct20X)
metric.list.NextSeq$threshold <- unlist(lapply(metric.list.NextSeq$TargetBasesPct20X, function(x, th=sd.TargetBasesPct20X, center=me.TargetBasesPct20X) {
  val <- x
  if (val > (center + (th* 2))) {
    return("High")
  }
...
metric.list.NextSeq.scaled$threshold <- unlist(lapply(metric.list.NextSeq$TargetBasesPct20X, function(x, th=sd.TargetBasesPct20X, center=me.TargetBasesPct20X) {
  val <- x
  if (val > (center + (th* 2))) {
    return("High")
  }
...
ggsave(
  "NextSeq_threshold_change_TargetBasesPct20X.png",
  plot = mean.lines,
  dpi = 300,
  limitsize = TRUE,
)
...
complete.data.edit$FractionDuplicates <- (1 - complete.data.edit$PFuniqueReads/complete.data.edit$TotalReads.x)
metric.list <- complete.data.edit %>% select(UniqueID,A260_280, A260_230, Phasing, PrePhasing, Intensity, ClusterDensity, PFmismatchRate, TargetBasesPct20X, OnBaitVSselected, ExcMapQPct, MedianTargetCoverage, HetSNPsensitivity, StrandBalance, StandardDeviation, ChimerasPercentage, PFUQaligned, MeanSize, FractionDuplicates, Instrument)
...
metric.list.NextSeq.scaled <- as.data.frame(apply(metric.list.NextSeq, 2, range02))
sd.PFmismatchRate <- sd(metric.list.NextSeq$PFmismatchRate)
me.PFmismatchRate <- mean(metric.list.NextSeq$PFmismatchRate)
metric.list.NextSeq$threshold <- unlist(lapply(metric.list.NextSeq$PFmismatchRate, function(x, th=sd.PFmismatchRate, center=me.PFmismatchRate) {
  val <- x
  if (val > (center + (th* 2))) {
    return("High")
  }
...
metric.list.NextSeq.scaled$threshold <- unlist(lapply(metric.list.NextSeq$PFmismatchRate, function(x, th=sd.PFmismatchRate, center=me.PFmismatchRate) {
  val <- x
  if (val > (center + (th* 2))) {
    return("High")
  }
...
ggsave(
  "NextSeq_threshold_change_PFmismatchRate.png",
  plot = mean.lines,
  dpi = 300,
  limitsize = TRUE,
)
