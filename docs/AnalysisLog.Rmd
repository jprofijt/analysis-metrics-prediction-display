...
dbDisconnect(connection)
rm(ASM,hs,InsertSizes,FL,CD,RunMetrics, RUNS, RunSummary, samples, res, connection, DNA_number, i, unique_combo, v, check_flowcell, instruments)
...
# Remove columns with 0 varance, as they do not provide any usable information
Numeric_data <- Numeric_data %>% select(names(variance.data[variance.data != 0])) 
head(Numeric_data)
```


```{r echo=FALSE}
length(Numeric_data$Yield) == sum(Numeric_data$Yield == Numeric_data$ProjectedYield)
```
Yield and Projected yield are the same in our instance so Projected yield will be removed from the data

```{r}
Numeric_data <- Numeric_data %>% select(!ProjectedYield)
head(Numeric_data)
```
Q30 is a percentage, so it should not be 380. In this instance we will be using Average_Q30

```{r}
Numeric_data <- Numeric_data %>% select(!Q30)
head(Numeric_data)
```
...
```{r}
library(reshape2)
interop <- Numeric_data %>% select(Yield, Aligned, Intensity, ClusterDensity, Average_Q30, Phasing, PrePhasing, ClusterPF)
ASM <- Numeric_data %>% select(TotalReads.x, PFreads.x, PFaligned, PFalignedBases, PFHQmedianMismatches, PFmismatchRate, PFHQErrorRate, PFindelRate, MeanReadLenght, ReadsAllignedInPairs, StrandBalance, ChimerasPercentage, AdapterPercentage)
...
hsm <- hsm[hsm %in% names(Numeric_data)]
HS <- Numeric_data %>% select(hsm)
HS1 <- HS[1:10]
...
ISM <- ISM[ISM %in% names(Numeric_data)]
IS <- Numeric_data %>% select(ISM)
...
FLM <- FLM[FLM %in% names(Numeric_data)]
FL <- Numeric_data %>% select(FLM)
