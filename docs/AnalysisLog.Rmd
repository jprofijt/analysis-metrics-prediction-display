...
```{r}
ncol(complete.data.edit[-c(1:15)])
nrow(complete.data.edit[-c(1:15)])
```

This leaves us with a dataset of 6960 samples, with 87 variables.

```{r}
nrow(complete.data.edit[complete.data.edit$Used == "Reinserted",])
nrow(complete.data.edit[complete.data.edit$Used == "Not Reinserted",])
```
...
model.data.all2$threshold <- model.data.all2$Used
model.data.all2.re <- model.data.all2 %>% select(-c(Used, Instrument))
re <- perform_t.tests(model.data.all2.re, test.low = F)
...
metric.list.labeled <- complete.data.edit %>% select(UniqueID,A260_280, A260_230, Intensity, PrePhasing, Phasing, ClusterDensity, Average_Q30, ClusterPF, ChimerasPercentage, FoldEnrichment, TargetBasesPct20X, OnBaitVSselected, PFmismatchRate, HetSNPsensitivity, MeanTargetCoverage, FractionDuplicates, PFindelRate, MeanSize, ExcMapQPct, SingletonsPercentage, StrandBalance, HsPenalty20X, Used)
...
metric.list.NextSeq.scaled[c(2,prebam,postbam)] <- as.data.frame(apply(metric.list.NextSeq.scaled[c(2,prebam,postbam)], 2, function(x) {
  rescale(x, to=c(-1,1))
}))
...
mean(na.omit(metric.list.NextSeq.outlier$FoldEnrichment[metric.list.NextSeq.outlier$threshold == "Norm"]))
```
...
```{r}
median(na.omit(metric.list.NextSeq.scaled[metric.list.NextSeq.outlier$threshold == "High",]$ChimerasPercentage))


median(na.omit(metric.list.NextSeq.scaled[metric.list.NextSeq.outlier$threshold == "Norm",]$ChimerasPercentage))

median(na.omit(metric.list.NextSeq.scaled[metric.list.NextSeq.outlier$threshold == "Low",]$ChimerasPercentage))
...
perform_t.tests <- function(metrics, test.high=T, test.low=T) {
  ...
  ThresholdTests.high <- data.frame(Metric=character(),
                   Significance=double(), 
                   Significant=logical(),
                   Difference_from_mean=character(), 
                   stringsAsFactors=FALSE)
  ...
  for (name in colnames(normal)) {
    ...
    if (test.high){
      ...
      #add to table
      ThresholdTests.high[nrow(ThresholdTests.high) + 1,] <- c(name,signif(t$p.value, digits = 3), t$p.value < 0.05, signif(mean(dat.high) - mean(dat.norm), digits = 3))
    }
    if (test.low){
    ...
    #add to table
    ThresholdTests.low[nrow(ThresholdTests.low) + 1,] <- c(name, signif(t$p.value, digits = 3), t$p.value < 0.05, signif(mean(dat.low) - mean(dat.norm), digits = 3))
    }
  }
  if (test.high){
    if (test.low){
...
library(kableExtra)
library(magick)
res <- perform_t.tests(na.omit(metric.list.NextSeq.outlier[-c(1:3)]))

r1 <- res[[1]][res[[1]]$Significant == T,]
r2 <- res[[2]][res[[2]]$Significant == T,]
r1
library(gridExtra)
png("test.png", height = 50*nrow(r1), width = 200*ncol(r2))
grid.table(r1[-c(3)], theme=ttheme_minimal())
dev.off()

library(gridExtra)
png("r2.png", height = 50*nrow(r2), width = 200*ncol(r2))
grid.table(r2[-c(3)], theme=ttheme_minimal())
dev.off()
```
```{r}
summary(metric.list.NextSeq$ClusterPF)
```
...
metric.list.MiSeq.scaled[c(2,prebam,postbam)] <- as.data.frame(apply(metric.list.MiSeq.scaled[c(2,prebam,postbam)], 2, function(x) {
  rescale(x, to=c(-1,1))
}))
...
```{r}
res <- perform_t.tests(na.omit(metric.list.MiSeq.outlier[-c(1:3)]))
...
```{r}
res <- perform_t.tests(na.omit(metric.list.NextSeq.outlier[-c(1:3)]), test.high = F)

r1 <- res[res$Significant == T,]
...
```{r}
res <- perform_t.tests(na.omit(metric.list.MiSeq.outlier[-c(1:3)]), test.high = F)
...
me.intens + sd.Intens * 2
sum(metric.list.NextSeq$Intensity < (me.intens - 7200*2))
...
metric.list.NextSeq.outlier <- metric.list.NextSeq
metric.list.NextSeq.outlier[metric.list.NextSeq.outlier$threshold == "High",][c(prebam,postbam)] <- as.data.frame(apply(metric.list.NextSeq.outlier[metric.list.NextSeq.outlier$threshold == "High",][c(prebam,postbam)], 2, remove_outliers_norm))
metric.list.NextSeq.outlier[metric.list.NextSeq.outlier$threshold == "High",]
...
metric.list.NextSeq.scaled <- na.omit(metric.list.NextSeq.outlier)
metric.list.NextSeq.scaled[c(4:(ncol(metric.list.NextSeq.scaled) -1))] <- as.data.frame(apply(metric.list.NextSeq.scaled[c(prebam,postbam)], 2, function(x) {
  rescale(x, to=c(-1,1))
}))
...
metric.list.NextSeq.scaled[metric.list.NextSeq.scaled$threshold == "High", ]
ggplot(metric.list.NextSeq.scaled, aes(x = metric.list.NextSeq.scaled$Intensity, y=metric.list.NextSeq.scaled$threshold))
...
```{r}
res <- perform_t.tests(na.omit(metric.list.NextSeq.outlier[-c(1:4)]), test.low = F)
...
metric.list.MiSeq.scaled[metric.list.MiSeq.scaled$threshold == "High",]
...
```{r}
res <- perform_t.tests(na.omit(metric.list.NextSeq.outlier[-c(1:3, 5,6)]), test.low = F)
...
res <- perform_t.tests(na.omit(metric.list.MiSeq.outlier[-c(1:3, 5,6)]), test.low = F)
...
metric.list.MiSeq.outlier$ClusterPF
...
res <- perform_t.tests(na.omit(metric.list.MiSeq.outlier[-c(1:3)]), test.low = F)
...
mean.lines <- ggplot(df_melted, aes(x = variable, y = value, fill = threshold)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_manual(values=c("red", "green", "blue"), name = "Threshold", labels = c("Above", "Between", "Below")) +
  ggtitle("Mean scaled metric change", subtitle = "compared to the Cluster density thresholds 1000-1200K/mm2") + 
  ylab("Change from center") + 
  theme(axis.text.x=element_text(angle = 90, size = 15),
        plot.title = element_text(size = 20),
        plot.subtitle = element_text(size = 15),
...
metric.list.labeled.outlier <- metric.list.labeled
metric.list.labeled.outlier[metric.list.labeled$Used == "Reinserted",][c(2:3,prebam,postbam)] <- as.data.frame(apply(metric.list.labeled[metric.list.labeled$Used == "Reinserted",][c(2:3,prebam,postbam)], 2, remove_outliers_norm))

metric.list.labeled.outlier[metric.list.labeled$Used == "Not Reinserted",][c(2:3,prebam,postbam)] <- as.data.frame(apply(metric.list.labeled[metric.list.labeled$Used == "Not Reinserted",][c(2:3,prebam,postbam)], 2, remove_outliers_norm))


metric.list.labeled.scaled <- na.omit(metric.list.labeled.outlier)
metric.list.labeled.scaled[c(2:(ncol(metric.list.MiSeq.scaled) -1))] <- as.data.frame(apply(metric.list.labeled.scaled[c(2:3,prebam,postbam)], 2, function(x) {
  rescale(x, to=c(-1,1))
}))

metric.list.labeled.scaled$Used <- factor(metric.list.labeled.scaled$Used, levels = c("Reinserted", "Not Reinserted"))
df_melted = melt(metric.list.labeled.scaled[-2], id.vars = c('UniqueID', 'Used'), na.rm = TRUE)
df_melted
mean.lines <- ggplot(df_melted, aes(x = variable, y = value, fill = Used)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_manual(values=c("green", "blue"), name = "Used", labels = c("Reinserted", "Not Reinserted")) +
  ggtitle("Mean scaled metric change", subtitle = "compared to the 260:230 Used limits (labeled)") + 
  ylab("Change from center") + 
  theme(axis.text.x=element_text(angle = 90, size = 15),
        plot.title = element_text(size = 20),
        plot.subtitle = element_text(size = 15),
